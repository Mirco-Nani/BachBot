...
class MidiDataset():
    def __init__(self, json_dataset):
        self.defaults = json_dataset["defaults"]
        self.encodings = json_dataset["encodings"]
        self.encodings_array = [self.encodings[k] for k in self.encodings]
        self.encoder = None
        
    def get_encoder(self):
        if self.encoder is None:
            self.encoder = MidiEncoder(defaults = self.defaults)
        return self.encoder
        
    def get_vocabulary_size(self):
        return len(self.get_encoder().get_vocabulary())
        
    def generate_sequence(self, encoder_size, decoder_size):
        r = random.randint(0, len(self.encodings_array)-1) #endpoints included
        random_song = self.encodings_array[r]
        
        total_size = encoder_size + decoder_size
        
        r = random.randint(0, len(random_song)-1 - total_size - 1)
        
        encoder_input = random_song[r:r+encoder_size]
        decoder_input = random_song[r+encoder_size:r+encoder_size+decoder_size]
        decoder_output = random_song[r+encoder_size+1:r+encoder_size+decoder_size+1]
        
        return encoder_input, decoder_input, decoder_output
    
    def generate_batch(self, encoder_size, decoder_size, batch_size):
        encoder_input = [] 
        decoder_input = []
        decoder_output = []
        for i in range(batch_size):
            enc_input, dec_input, dec_output = self.generate_sequence(encoder_size, decoder_size)
            encoder_input.append(enc_input)
            decoder_input.append(dec_input)
            decoder_output.append(dec_output)
        return encoder_input, decoder_input, decoder_output
    
    def generate_np_batch(self, encoder_size, decoder_size, batch_size):
        encoder_input, decoder_input, decoder_output = self.generate_batch(encoder_size, decoder_size, batch_size)
